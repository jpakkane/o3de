...
#define AZ_COMPILER_GNU __GNUC__
...
#define AZ_PUSH_DISABLE_WARNING(_msvcOption, __) __pragma(warning(push)) __pragma(warning(disable : _msvcOption))

/// Pops the warning stack. For use matched with an AZ_PUSH_DISABLE_WARNING
#define AZ_POP_DISABLE_WARNING __pragma(warning(pop))
...
/// Disables a warning for dll exported classes which has non dll-exported members as this can cause ABI issues if the layout of those
/// classes differs between dlls. QT classes such as QList, QString, QMap, etc... and Cry Math classes such Vec3, Quat, Color don't
/// dllexport their interfaces Therefore this macro can be used to disable the warning when caused by 3rdParty libraries
...
#if !defined(_DEBUG)
#pragma warning(disable : 4714) // warning C4714 marked as __forceinline not inlined. Sadly this happens when LTCG during linking. We tried
                                // to NOT use force inline but VC 2012 is bad at inlining.
#endif

/// Aligns a declaration.
#define AZ_ALIGN(_decl, _alignment)                                                                                                        \
    AZ_PUSH_DISABLE_WARNING(4324, "-Wunknown-warning-option")                                                                              \
    __declspec(align(_alignment)) _decl AZ_POP_DISABLE_WARNING
...
#define AZ_PUSH_DISABLE_WARNING(__, _clangOption)                                                                                          \
    _Pragma("clang diagnostic push") _Pragma(AZ_STRINGIZE(clang diagnostic ignored _clangOption))

/// Pops the warning stack. For use matched with an AZ_PUSH_DISABLE_WARNING
#define AZ_POP_DISABLE_WARNING _Pragma("clang diagnostic pop")
...
#define AZ_ALIGN(_decl, _alignment)                                                                                                        \
    AZ_PUSH_DISABLE_WARNING(4324, "-Wunknown-warning-option")                                                                              \
    _decl __attribute__((aligned(_alignment)))
