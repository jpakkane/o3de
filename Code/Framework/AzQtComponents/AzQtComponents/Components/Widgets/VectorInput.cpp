...
namespace AzQtComponents
{
    ...
    {
        m_spinBox->setKeyboardTracking(
            false); // don't send valueChanged every time a character is typed (no need for undo/redo per digit of a double value)
    ...
    QRect VectorElement::editFieldRect(
        const QProxyStyle* style, const QStyleOptionComplex* option, const QWidget* widget, const SpinBox::Config& config)
    {
        ...
        : QWidget(parent)
    {
        ...
        AZ_Warning("Property Editor", elementCount >= 2, "Vector controls with less than 2 elements are not supported");
        ...
        int numberOfRowsInLayout =
            elementsPerRow <= 0 ? 1 : static_cast<int>(std::ceil(static_cast<float>(m_elementCount) / static_cast<float>(elementsPerRow)));
        ...
        for (int rowIdx = 0; rowIdx < numberOfRowsInLayout; rowIdx++)
        {
            ...
            for (int columnIdx = 0; columnIdx < actualElementsPerRow; columnIdx++)
            {
                if (numberOfElementsRemaining > 0)
                {
                    ...
                    connect(
                        m_elements[elementIndex]->GetSpinBox(), valueChanged, this,
                        [elementIndex, this](double value)
                        {
    ...
    {
        AZ_Warning(
            "PropertyGrid", index < m_elementCount, "This control handles only %i controls", m_elementCount) if (index < m_elementCount)
        {
    ...
    {
        AZ_Warning(
            "PropertyGrid", elementIndex < m_elementCount, "This control handles only %i controls",
            m_elementCount) if (elementIndex < m_elementCount)
        {
    ...
    void VectorInput::setMinimum(double value)
    {
        for (size_t i = 0; i < m_elementCount; ++i)
        {
            ...
            // Avoid setting this value if its close to the current value, because otherwise Qt will pump events into the queue to
            // redraw/etc.
