...
namespace AzQtComponents
{
    ...
    public:
        ...
        bool isEditing(const QAbstractSpinBox* spinBox) const
        {
            return m_spinBoxChanging == spinBox;
        }
        ...
        case QEvent::Leave:
            {
                if (auto lineEdit = spinBox->findChild<QLineEdit*>(QString(), Qt::FindDirectChildrenOnly))
                {
                    ...
                    const auto newGeometry =
                        spinBox->style()->subControlRect(QStyle::CC_SpinBox, &styleOption, QStyle::SC_SpinBoxEditField, spinBox);
        ...
        case QEvent::MouseButtonPress:
            {
                ...
                if (mouseEvent->button() == Qt::LeftButton)
                {
                    ...
                    initStyleOption(spinBox, &styleOption);
                    const auto control =
                        spinBox->style()->hitTestComplexControl(QStyle::CC_SpinBox, &styleOption, mouseEvent->pos(), spinBox);
    ...
    bool SpinBoxWatcher::filterLineEditEvents(QLineEdit* lineEdit, QEvent* event)
    {
        ...
        case QEvent::Paint:
            {
                if (lineEdit->property(g_spinBoxIntializedValueName).isValid() &&
                    !lineEdit->property(g_spinBoxIntializedValueName).toBool())
                {
                    return true;
                }
                break;
        ...
        case QEvent::MouseButtonDblClick:
            {
                ...
                if (m_config.autoSelectAllOnClickFocus)
                {
                    if (mouseEvent->button() == Qt::LeftButton && m_mouseFocusedSpinBoxSingleClicked == spinBox)
                    {
                        ...
                        QMouseEvent fake(
                            QEvent::MouseButtonPress, mouseEvent->localPos(), mouseEvent->button(), mouseEvent->buttons(),
    ...
    void SpinBoxWatcher::resetCursor(QAbstractSpinBox* spinBox)
    {
        ...
        const QStyle::SubControl control = spinBox->style()->hitTestComplexControl(QStyle::CC_SpinBox, &styleOption, pos, spinBox);
    ...
    bool SpinBox::drawSpinBox(
        const QProxyStyle* style, const QStyleOption* option, QPainter* painter, const QWidget* widget, const SpinBox::Config& config)
    {
        Q_UNUSED(config);
    ...
    QRect SpinBox::editFieldRect(const QProxyStyle* style, const QStyleOptionComplex* option, const QWidget* widget, const Config& config)
    {
        ...
        if (auto spinBoxOption = qstyleoption_cast<const QStyleOptionSpinBox*>(option))
        {
            ...
            const auto baseRect = style->baseStyle()->subControlRect(QStyle::CC_SpinBox, option, QStyle::SC_SpinBoxEditField, widget);
            const auto buttonRect = style->baseStyle()->subControlRect(QStyle::CC_SpinBox, option, QStyle::SC_SpinBoxUp, widget);
            QRect rect(baseRect);
            rect.setRight(buttonRect.x());
            rect.adjust(0, 0, -1, 0); // Don't overlap buttonRect or the spinbox border

            if (!(spinBoxOption->state & QStyle::State_Enabled) || !(spinBoxOption->state & QStyle::State_MouseOver))
            {
        ...
        : QSpinBox(parent)
    {
        ...
        connect(
            this, &SpinBox::cutTriggered, this,
            [this]()
            {
        ...
        connect(
            this, &SpinBox::copyTriggered, this,
            [this]()
            {
                m_lineEdit->copy();
            });
        connect(
            this, &SpinBox::pasteTriggered, this,
            [this]()
            {
                setInitialValueWasSetting(true);
        ...
        connect(
            this, &SpinBox::deleteTriggered, this,
            [this]()
            {
                setInitialValueWasSetting(true);
                m_lineEdit->del();
    ...
    {
        // We want to override the context menu's undo/redo in the case that the lineEdit control does not currently have any undo/redo
        // available but the QAbstractSpinBox doesn't really expose any way to do that nicely. As a result, we have to replicate the context
        // menu code from QAbstractSpinBox instead, and also pull out the undo and redo actions so that we can override them.
        ...
        if (QMenu* menu = lineEdit->createStandardContextMenu())
        {
            ...
            QObject::connect(
                upAction, &QAction::triggered, spinBox,
                [spinBox]
                {
            ...
            QObject::connect(
                downAction, &QAction::triggered, spinBox,
                [spinBox]
                {
                    spinBox->setInitialValueWasSetting(true);
                    spinBox->stepBy(-1);
            ...
            const QPoint pos = (ev->reason() == QContextMenuEvent::Mouse)
                ? ev->globalPos()
                : spinBox->mapToGlobal(QPoint(ev->pos().x(), 0)) + QPoint(spinBox->width() / 2, spinBox->height() / 2);
        ...
        , m_displayDecimals(g_decimalDisplayPrecisionDefault)
    {
        ...
        connect(
            this, &DoubleSpinBox::cutTriggered, this,
            [this]()
            {
                setInitialValueWasSetting(true);
                m_lineEdit->cut();
        ...
        connect(
            this, &DoubleSpinBox::copyTriggered, this,
            [this]()
            {
                m_lineEdit->copy();
            });
        connect(
            this, &DoubleSpinBox::pasteTriggered, this,
            [this]()
            {
                setInitialValueWasSetting(true);
                m_lineEdit->setText(QString());
        ...
        connect(
            this, &DoubleSpinBox::deleteTriggered, this,
            [this]()
            {
                setInitialValueWasSetting(true);
                m_lineEdit->del();
    ...
    namespace internal
    {
        ...
        bool SpinBoxLineEdit::event(QEvent* ev)
        {
            ...
            case QEvent::FocusOut:
                {
                    // Explicitly set the text again on focusOut, so that the undo/redo queue for the line edit clears and the global
                    // undo/redo can kick in
