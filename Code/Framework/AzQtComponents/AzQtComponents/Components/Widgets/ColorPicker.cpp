...
namespace AzQtComponents
{
    ...
    {
    public:
        CurrentColorChangedCommand(
            ColorPicker* picker, const AZ::Color& previousColor, const AZ::Color& newColor, QUndoCommand* parent = nullptr);
    ...
    ColorPicker::CurrentColorChangedCommand::CurrentColorChangedCommand(
        ColorPicker* picker, const AZ::Color& previousColor, const AZ::Color& newColor, QUndoCommand* parent)
        : QUndoCommand(parent)
        , m_picker(picker)
    ...
    public:
        PaletteAddedCommand(
            ColorPicker* picker, QSharedPointer<PaletteCard> card, ColorLibrary colorLibrary, QUndoCommand* parent = nullptr);
    ...
    ColorPicker::PaletteAddedCommand::PaletteAddedCommand(
        ColorPicker* picker, QSharedPointer<PaletteCard> card, ColorLibrary colorLibrary, QUndoCommand* parent)
        : QUndoCommand(parent)
        , m_picker(picker)
        , m_card(card)
    ...
    {
        QObject::connect(
            card.data(), &PaletteCard::contextMenuRequested, picker,
            [picker, card](const QPoint& point)
            {
    ...
    {
        return ColorPicker::Config{ 16, // Padding
                                    8, // Spacing;
                                    {
                                        // ColorGrid
    ...
    void ColorPicker::polish(const ColorPicker::Config& config)
    {
        ...
        m_containerWidget->layout()->setContentsMargins(
            { m_config.padding, 0, m_config.padding - m_scrollArea->verticalScrollBar()->sizeHint().width(), m_config.padding });
        ...
        , m_defaultLForHsMode(0.85)
    {
        ...
        connect(
            m_currentColorController, &Internal::ColorController::colorChanged, this,
            [this]()
            {
        ...
        m_alphaSlider->setColorFunction(
            [this](qreal position)
            {
        ...
        m_alphaSlider->setToolTipFunction(
            [this](qreal position)
            {
        ...
        connect(
            m_alphaSlider, &QSlider::valueChanged, this,
            [this](int alpha)
            {
        ...
        connect(
            m_currentColorController, &Internal::ColorController::alphaChanged, this,
            [this](qreal alpha)
            {
        ...
        m_hueSlider->setColorFunction(
            [](qreal position)
            {
                return toQColor(Internal::ColorController::fromHsv(position, 1.0, 1.0));
            });
        m_hueSlider->setToolTipFunction(
            [this](qreal position)
            {
                QString prefix = QStringLiteral("Hue: %0").arg(qRound(position * m_hueSlider->maximum()));
                return rgbToolTip(
                    prefix,
                    toQColor(Internal::ColorController::fromHsv(
                        position, m_currentColorController->hsvSaturation(), m_currentColorController->value())));
            });
        m_hueSlider->setMinimum(0);
        m_hueSlider->setMaximum(360);

        connect(
            m_hueSlider, &QSlider::valueChanged, this,
            [this](int value)
            {
        ...
        connect(
            m_currentColorController, &Internal::ColorController::hsvHueChanged, this,
            [this](qreal hue)
            {
        ...
        m_valueSlider->setColorFunction(
            [this](qreal position)
            {
                return toQColor(Internal::ColorController::fromHsv(
                    m_currentColorController->hsvHue(), m_currentColorController->hsvSaturation(), position));
            });
        m_valueSlider->setToolTipFunction(
            [this](qreal position)
            {
        ...
        connect(
            m_valueSlider, &QSlider::valueChanged, this,
            [this](int value)
            {
        ...
        connect(
            m_currentColorController, &Internal::ColorController::valueChanged, this,
            [this](qreal value)
            {
        ...
        m_eyedropperButton->setToolTip(tr(
            "Left click on this and hold the button down. On left mouse button release, the color under the mouse cursor will be picked."));
        m_eyedropper = new Eyedropper(this, m_eyedropperButton);
        connect(
            m_eyedropper, &Eyedropper::colorSelected, this,
            [this](const QColor& color)
            {
                setCurrentColor(FromQColor(color));
            });
        ...
        connect(
            m_toggleHueGridButton, &QAbstractButton::toggled, this,
            [this](bool checked)
            {
        ...
        connect(
            m_quickPaletteCard, &QuickPaletteCard::selectedColorsChanged, this,
            [this](const QVector<AZ::Color>& selectedColors)
            {
        ...
        connect(
            m_preview, &ColorPreview::colorSelected, m_quickPaletteCard,
            [this](const AZ::Color& color)
            {
        ...
        connect(
            m_paletteCardCollection, &PaletteCardCollection::savePaletteClicked, this,
            [this](QSharedPointer<PaletteCard> palette)
            {
                savePalette(palette, false);
            });
        connect(
            m_paletteCardCollection, &PaletteCardCollection::savePaletteAsClicked, this,
            [this](QSharedPointer<PaletteCard> palette)
            {
                savePalette(palette, true);
            });
        connect(
            m_paletteCardCollection, &PaletteCardCollection::paletteCountChanged, this,
            [this]
            {
        ...
        enableSaveRestoreGeometry(
            QStringLiteral("%1/%2/%3").arg(m_context).arg(ConfigurationName(m_configuration)).arg(g_windowGeometryKey));
    ...
    {
        const auto changedCards = std::any_of(
            m_colorLibraries.keyBegin(), m_colorLibraries.keyEnd(),
            [](const QSharedPointer<PaletteCard>& card)
            {
        ...
        if (changedCards)
        {
            ...
            int messageBoxResult = QMessageBox::question(
                this, tr("Color Picker"), tr("There are palettes with unsaved changes. Would you like to save them now?"), buttons,
                defaultButton);
    ...
    void ColorPicker::initContextMenu(ColorPicker::Configuration configuration)
    {
        ...
        if (configuration != Configuration::HueSaturation)
        {
            ...
            QObject::connect(
                m_showRgbValuesAction, &QAction::toggled,
                [this](bool checked)
                {
        ...
        connect(
            smallSwatches, &QAction::toggled, this,
            [this](bool checked)
            {
        ...
        connect(
            mediumSwatches, &QAction::toggled, this,
            [this](bool checked)
            {
        ...
        connect(
            largeSwatches, &QAction::toggled, this,
            [this](bool checked)
            {
        ...
        connect(
            m_toggleQuickPaletteAction, &QAction::triggered, this,
            [this]()
            {
    ...
    void ColorPicker::removePaletteCardRequested(QSharedPointer<PaletteCard> card)
    {
        if (card->modified())
        {
            int result = QMessageBox::question(
                this, tr("Color Picker"), tr("There are unsaved changes to your palette. Are you sure you want to close?"));
    ...
    void ColorPicker::addPalette(
        QSharedPointer<Palette> palette, const QString& fileName, const QString& title, const Internal::ColorLibrarySettings& settings)
    {
    ...
    void ColorPicker::showPreviewContextMenu(const QPoint& p, const AZ::Color& selectedColor)
    {
        ...
        connect(
            quickPaletteAction, &QAction::triggered, this,
            [this, selectedColor]()
            {
        ...
        if (paletteCount > 0)
        {
            ...
            for (int paletteIndex = 0; paletteIndex < paletteCount; paletteIndex++)
            {
                ...
                connect(
                    namedPaletteAction, &QAction::triggered,
                    [paletteCard, selectedColor]()
                    {
    ...
    void ColorPicker::paletteContextMenuRequested(QSharedPointer<PaletteCard> paletteCard, const QPoint& point)
    {
        ...
        connect(
            moveDown, &QAction::triggered, this,
            [&paletteCard, this]()
            {
    ...
    {
        auto it = std::find_if(
            m_colorLibraries.begin(), m_colorLibraries.end(),
            [palette](const ColorLibrary& entry)
            {
                return entry.palette.data() == palette;
            });
    ...
    void ColorPicker::readSettings()
    {
        ...
        for (int i = 0; i < colorLibrariesCount; ++i)
        {
            ...
            colorLibraries.push_back(std::make_pair(
                settings.value(g_colorLibraryNameKey).toString(),
        ...
        {
            QMessageBox::warning(
                this, tr("Failed to load color libraries"),
                tr("The following color libraries could not be located on disk:\n%1\n"
                   "They will be removed from your saved settings. Please re-import them again if you can locate them.")
                    .arg(missingLibraries.join('\n')));
        }
    ...
    void ColorPicker::writeSettings() const
    {
        ...
        auto swatchSizeActions = m_swatchSizeGroup->actions();
        auto it = std::find_if(
            swatchSizeActions.begin(), swatchSizeActions.end(),
            [](QAction* action)
            {
