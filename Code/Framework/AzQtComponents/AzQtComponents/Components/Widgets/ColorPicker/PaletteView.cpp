...
namespace AzQtComponents
{
    ...
    public:
        ...
        void insertIgnoringDuplicates(
            int row, QVector<AZ::Color>::const_iterator first, QVector<AZ::Color>::const_iterator last, bool isUndoRedo = false);
    ...
    namespace UndoRedo
    {
        ...
        PaletteModelInsertionCommand::PaletteModelInsertionCommand(
            PaletteModel& target, int index, const QVector<AZ::Color>& colors, QUndoCommand* parent)
            : QUndoCommand(parent)
            , m_target(target)
            , m_index(index)
            , m_colors(colors)
        ...
        PaletteModelSetColorCommand::PaletteModelSetColorCommand(
            PaletteModel& target, int index, const AZ::Color& color, QUndoCommand* parent)
            : QUndoCommand(parent)
            , m_target(target)
            , m_index(index)
    ...
    QVector<AZ::Color> PaletteModel::colorsFromIndices(const QModelIndexList& indices) const
    {
        ...
        std::transform(
            indices.cbegin(), indices.cend(), std::back_inserter(colors),
            [](QModelIndex i)
            {
                return i.data(PaletteModel::ColorRole).value<AZ::Color>();
            });
        return colors;
    }
    ...
    bool PaletteModel::lacksAnyOf(const QVector<AZ::Color>& colors) const
    {
        return std::any_of(
            ...
            {
                return std::none_of(
                    m_palette->colors().cbegin(), m_palette->colors().cend(),
                    [color](const AZ::Color& existing)
                    {
    ...
    int PaletteModel::indexOf(const AZ::Color& color) const
    {
        auto i = std::find_if(
            m_palette->colors().cbegin(), m_palette->colors().cend(),
            [&color](const AZ::Color& candidate)
            {
                return candidate.IsClose(color);
            });
    ...
    bool PaletteModel::tryInsert(
        int row, QVector<AZ::Color>::const_iterator first, QVector<AZ::Color>::const_iterator last, bool isUndoRedo)
    {
        insertIgnoringDuplicates(row, first, last, isUndoRedo);
        return true;
    }

    void PaletteModel::insertIgnoringDuplicates(
        int row, QVector<AZ::Color>::const_iterator first, QVector<AZ::Color>::const_iterator last, bool isUndoRedo)
    {
        if (!isUndoRedo)
        {
        ...
        , m_addColorButton(new Internal::AddSwatch(viewport()))
    {
        ...
        connect(
            m_addColorButton, &Internal::AddSwatch::clicked, this,
            [this]()
            {
        ...
        m_update = m_context->addAction(
            tr("Update to current"), this,
            [this]()
            {
    ...
    void PaletteView::addDefaultRGBColors()
    {
        ...
        QVector<AZ::Color> colors({ Color(u8(0xBF), u8(0x28), u8(0x28), u8(0xff)), Color(u8(0xE9), u8(0xA9), u8(0x44), u8(0xff)),
                                    Color(u8(0x84), u8(0x59), u8(0x32), u8(0xff)), Color(u8(0x92), u8(0xD1), u8(0x48), u8(0xff)),
                                    Color(u8(0xAD), u8(0x29), u8(0xD8), u8(0xff)), Color(u8(0x9B), u8(0x9B), u8(0x9B), u8(0xff)),
                                    Color(u8(0xB4), u8(0xCA), u8(0x9A), u8(0xff)), Color(u8(0x6E), u8(0x5D), u8(0x81), u8(0xff)),
                                    Color(u8(0x77), u8(0xCF), u8(0xA8), u8(0xff)), Color(u8(0x53), u8(0x55), u8(0x51), u8(0xff)),
    ...
    void PaletteView::handleSelectionChanged(const QItemSelection& selected, const QItemSelection& deselected)
    {
        ...
        if (!m_aboutToShowContextMenu)
        {
            ...
            std::transform(
                selection.cbegin(), selection.cend(), std::back_inserter(selectedColors),
                [](const QModelIndex i) -> AZ::Color
                {
                    return i.data(PaletteModel::ColorRole).value<AZ::Color>();
                });
    ...
    void PaletteView::updateItemLayout()
    {
        ...
        const int width = viewport()->contentsMargins().left() + m_itemColumns * (m_itemSize.width() + m_itemHSpacing) -
            (m_itemColumns == 0 ? 0 : m_itemHSpacing) + viewport()->contentsMargins().right();
        const int height = viewport()->contentsMargins().top() + m_itemRows * (m_itemSize.height() + m_itemVSpacing) -
            (m_itemRows == 0 ? 0 : m_itemVSpacing) + viewport()->contentsMargins().bottom();
    ...
    void PaletteView::removeSelection()
    {
        ...
        std::sort(
            ...
            [](const QItemSelectionRange& l, const QItemSelectionRange& r)
            {
                return l.top() > r.top();
            });
    ...
    void PaletteView::paste()
    {
        ...
        while (pasted != pastedColors.end())
        {
            ...
            {
                if (std::none_of(
                        selectedColors.cbegin(), selectedColors.cend(),
                        [pasted](AZ::Color c)
                        {
                            return c.IsClose(*pasted);
                        }))
                {
    ...
    bool PaletteView::drawDropIndicator(
        const Style* style, const QStyleOption* option, QPainter* painter, const QWidget* widget, const PaletteView::Config& config)
    {
        Q_UNUSED(style);
        ...
        if (view->m_droppingOn)
        {
            ...
            QRect adjusted = option->rect.adjusted(
                -config.dropIndicator.width, -config.dropIndicator.width, config.dropIndicator.width, config.dropIndicator.width);
