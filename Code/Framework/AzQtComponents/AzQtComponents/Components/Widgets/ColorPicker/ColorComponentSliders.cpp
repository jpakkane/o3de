...
namespace AzQtComponents
{
    namespace
    {
        ...
        {
            const QString labels[] = { QObject::tr("H"), QObject::tr("S"), QObject::tr("L"), QObject::tr("V"),
                                       QObject::tr("R"), QObject::tr("G"), QObject::tr("B") };
        ...
        QString createToolTipText(QString prefix, qreal position, SliderType* slider, bool includeRGB = true)
        {
            if (includeRGB)
            {
                ...
                return QStringLiteral("%0: %1\nRGB: %2, %3, %4")
                    .arg(prefix)
                    .arg(qRound(position * slider->maximum()))
                    .arg(rgb.red())
                    .arg(rgb.green())
                    .arg(rgb.blue());
            }
        ...
        , m_defaultLForHsMode(0.85)
    {
        ...
        m_saturationSlider->setColorFunction(
            [this](qreal position)
            {
                using namespace AzQtComponents::Internal;
        ...
        m_saturationSlider->setToolTipFunction(
            [this](qreal position)
            {
                return createToolTipText("Saturation", position, m_saturationSlider);
            });

        m_lightnessSlider->setColorFunction(
            [this](qreal position)
            {
                using namespace AzQtComponents::Internal;
        ...
        m_lightnessSlider->setToolTipFunction(
            [this](qreal position)
            {
        ...
        : QWidget(widget)
    {
        ...
        m_redSlider->setColorFunction(
            [this](qreal position)
            {
                return toQColor(
                    aznumeric_cast<float>(position), aznumeric_cast<float>(m_greenSlider->value()),
                    aznumeric_cast<float>(m_blueSlider->value()));
            });
        m_redSlider->setToolTipFunction(
            [this](qreal position)
            {
                return createToolTipText("Red", position, m_redSlider, /*includeRGB=*/false);
            });

        m_greenSlider->setColorFunction(
            [this](qreal position)
            {
                return toQColor(
                    aznumeric_cast<float>(m_redSlider->value()), aznumeric_cast<float>(position),
                    aznumeric_cast<float>(m_blueSlider->value()));
            });
        m_greenSlider->setToolTipFunction(
            [this](qreal position)
            {
                return createToolTipText("Green", position, m_greenSlider, /*includeRGB=*/false);
            });

        m_blueSlider->setColorFunction(
            [this](qreal position)
            {
                return toQColor(
                    aznumeric_cast<float>(m_redSlider->value()), aznumeric_cast<float>(m_greenSlider->value()),
                    aznumeric_cast<float>(position));
            });
        m_blueSlider->setToolTipFunction(
            [this](qreal position)
            {
