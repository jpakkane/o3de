...
namespace AzQtComponents
{
    ...
    void Slider::showHoverToolTip(
        const QString& toolTipText,
        const QPoint& globalPosition,
        QSlider* slider,
        QWidget* toolTipParentWidget,
        int width,
        int height,
        const QPoint& toolTipOffset)
    {
    ...
    QRect Slider::sliderHandleRect(
        const Style* style, const QStyleOptionSlider* option, const QWidget* widget, const Config& config, bool isHovered)
    {
        Q_ASSERT(widget);
    ...
    bool Slider::drawSlider(
        const Style* style, const QStyleOption* option, QPainter* painter, const QWidget* widget, const Slider::Config& config)
    {
        ...
        if (slider != nullptr)
        {
            ...
            else if (auto sliderWidget = qobject_cast<const Slider*>(widget->parentWidget()))
            {
                const QColor handleColor =
                    option->state & QStyle::State_Enabled ? config.slider.handle.color : config.slider.handle.colorDisabled;
                ...
                if (option->state & QStyle::State_Enabled)
                {
                    ...
                    {
                        QPoint valueGrooveStart = orientation == Qt::Horizontal
                            ? QPoint(visibleGrooveRect.center().x(), visibleGrooveRect.top())
                            : QPoint(visibleGrooveRect.left(), visibleGrooveRect.center().y());
                        QPoint handleMiddle = orientation == Qt::Horizontal ? QPoint(handleRect.x(), visibleGrooveRect.bottom())
                                                                            : QPoint(visibleGrooveRect.right(), handleRect.bottom());
                    ...
                    {
                        QPoint valueGrooveStart =
                            orientation == Qt::Horizontal ? visibleGrooveRect.topLeft() : visibleGrooveRect.bottomLeft();
                        QPoint valueGrooveEnd = orientation == Qt::Horizontal ? QPoint(handleRect.x(), visibleGrooveRect.bottom())
                                                                              : QPoint(visibleGrooveRect.right(), handleRect.y());
                ...
                else
                {
                    ...
                    {
                        firstGrooveRect =
                            QRect(visibleGrooveRect.topLeft(), QPoint(handleRect.left() - disabledGap, visibleGrooveRect.bottom()));
                        lastGrooveRect =
                            QRect(QPoint(handleRect.right() + disabledGap, visibleGrooveRect.top()), visibleGrooveRect.bottomRight());
                    }
                    else
                    {
                        firstGrooveRect =
                            QRect(visibleGrooveRect.topLeft(), QPoint(visibleGrooveRect.right(), handleRect.top() - disabledGap));
                        lastGrooveRect =
                            QRect(QPoint(visibleGrooveRect.left(), handleRect.bottom() + disabledGap), visibleGrooveRect.bottomRight());
                    }
                ...
                if (drawHighlights)
                {
                    if (style->hasClass(widget, g_midPointStyleClass))
                    {
                        drawMidPointGrooveHighlights(
                            painter, config, visibleGrooveRect, handleRect, sliderWidget->m_mousePos, sliderOption->orientation);
                    }
                    else
                    {
                        drawGrooveHighlights(
                            painter, config, visibleGrooveRect, handleRect, sliderWidget->m_mousePos, sliderOption->orientation);
                    }
                }
    ...
    void Slider::drawMidPointMarker(
        QPainter* painter, const Slider::Config& config, Qt::Orientation orientation, const QRect& grooveRect, const QColor& midMarkerColor)
    {
    ...
    void Slider::drawMidPointGrooveHighlights(
        QPainter* painter,
        const Slider::Config& config,
        const QRect& grooveRect,
        const QRect& handleRect,
        const QPoint& mousePos,
        Qt::Orientation orientation)
    {
        QPoint valueGrooveStart = orientation == Qt::Horizontal ? QPoint(grooveRect.center().x(), grooveRect.top())
                                                                : QPoint(grooveRect.left(), grooveRect.center().y());
    ...
    void Slider::drawGrooveHighlights(
        QPainter* painter,
        const Slider::Config& config,
        const QRect& grooveRect,
        const QRect& handleRect,
        ...
        Qt::Orientation orientation)
    {
        ...
        QPoint valueGrooveEnd =
            orientation == Qt::Horizontal ? QPoint(handleRect.x(), grooveRect.bottom()) : QPoint(grooveRect.right(), handleRect.y());
    ...
    void Slider::drawHandle(
        const QStyleOption* option, QPainter* painter, const Slider::Config& config, const QRect& handleRect, const QColor& handleColor)
    {
        // handle
        Q_UNUSED(config);
        painter->save();
    ...
    {
        connect(
            m_slider, &QSlider::valueChanged, this,
            [this](int newValue)
            {
    ...
    double SliderDouble::convertPowerCurveValue(double value, bool fromSlider) const
    {
        ...
        else
        {
            ...
            // Power curve variables based on the various slider min, midpoint, and max values
            const double a =
                ((sliderMin * sliderMax) - (sliderMidpoint * sliderMidpoint)) / (sliderMin - (2.0 * sliderMidpoint) + sliderMax);
