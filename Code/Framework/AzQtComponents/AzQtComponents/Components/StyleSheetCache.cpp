...
namespace AzQtComponents
{
    ...
    void StyleSheetCache::registerPathsFoundOnDisk(const QString& pathOnDisk, const QString& qrcPrefix)
    {
        ...
        while (!entriesToScan.empty())
        {
            ...
            {
                for (auto subEntry :
                     QDir(entry.absoluteFilePath()).entryInfoList({ "*.qss" }, QDir::NoDotAndDotDot | QDir::Dirs | QDir::Files))
                {
            ...
            else
            {
                ...
                QString qrcPath = QStringLiteral("%1/%2").arg(qrcPrefix).arg(rootDirectory.relativeFilePath(diskPath));
    ...
    void StyleSheetCache::addSearchPaths(
        const QString& searchPrefix, const QString& pathOnDisk, const QString& qrcPrefix, const AZ::IO::PathView& engineRootPath)
    {
        AZ_Warning(
            "StyleSheetCache", m_prefixes.find(searchPrefix) == m_prefixes.end(), "Style prefix \"%s\" was already registered, ignoring...",
            searchPrefix.constData());
    ...
    QString StyleSheetCache::loadStyleSheet(QString styleFileName)
    {
        ...
        if (filePathInfo.exists() && filePathInfo.isNativePath())
        {
            ...
            {
                AZ_Error(
                    "StyleSheetCache", false, "No QRC entry was found for style-sheet loaded from disk, loading has been disabled: %s",
